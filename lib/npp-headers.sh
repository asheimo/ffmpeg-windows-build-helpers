#!/usr/bin/env bash
# npp-headers.sh — installs NVIDIA Performance Primitives (NPP) headers,
...
[[ -n "${NPP_HEADERS_SH_LOADED:-}" ]] && return
NPP_HEADERS_SH_LOADED=1
...
build_npp_headers() {
  ...
  local touch_name
  touch_name="$(get_touchfile_name "already_installed_npp_headers_${libs_version}")"
