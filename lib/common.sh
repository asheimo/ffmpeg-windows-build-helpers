...
run_library() {
  ...
  if ( cd "$BUILD_DIR" && "$build_fn" 2>&1 | tee "$lib_log" | { [[ "${BUILD_VERBOSE:-y}" == "y" ]] && cat || grep -E "^\s+\[(git|download|configure|cmake|make|patch|ar|install|dlltool|deps)\]|ERROR|FAILED|warning:"; } ); then
